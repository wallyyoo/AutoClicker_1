       ...
       if (File.Exists(path))
            {
                ...
                //  여기에 아이템 상태 덮어쓰기
                foreach (var jsonItem in saveData.equipped)
                {
                    var item = System.Array.Find(ItemManager.ItemManagerInstance.allItems,
                        x => x.itemId == jsonItem.itemId);
                    if (item != null)
                    {
                        item.isPurchased = jsonItem.isPurchased;
                        item.isEquipped = jsonItem.isEquipped;
                        item.upgradeLevel = jsonItem.upgradeLevel;
                    }
                }
            ...
            {
                Debug.Log("저장된 파일이 없습니다. ItemData를 초기화합니다.");

                // 여기서 ItemData 전체 초기화
                if (ItemManager.ItemManagerInstance != null)
                {
                    ItemManager.ItemManagerInstance.ResetAllItemData();
                }
